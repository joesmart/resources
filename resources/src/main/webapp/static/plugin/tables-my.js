...
(function ($, window, document, undefined) {
    ...
    function createPagination(options, current_page) {
        ...
        var start = options.first_page_no > 1 ? options.first_page_no : 1;
        var end = options.pagination_length;


        start = current_page - 2 <= 0 ? 1 : current_page - 2;
        if(start == 1){
            end =  options.pagination_length;
        }else{
            end = start + options.pagination_length;
        }


        if (end > options.page_info.totalPage) {
            end = options.page_info.totalPage;
        }
        options.first_page_no = start;
        options.last_page_no = end;

        if (options.first_page_no != 1) {
            li = $("<li/>").append($("<span/>").text('...').attr('href', 'javascript:void(0)'));
        ...
        if (options.last_page_no != options.page_info.totalPage) {
            li = $("<li/>").append($("<span/>").text('...').attr('href', 'javascript:void(0)'));
            pagination.append(li);
        }
        return pagination;
    ...
    $.fn.myTables = function (options) {
        ...
        return this.each(function () {
            ...
            $(".pagination ul li a").live("click", function () {
                ...
                var updateOne = true;
                if (options.last_page_no - current_page <= 3) {
                    pagination.remove();
                    pagination = createPagination(options, current_page);
                    _div_pagination.append(pagination);
                    updateOne = false;
                }

                if (current_page - options.first_page_no <= 3 && updateOne) {
                    pagination.remove();
                    pagination = createPagination(options, current_page);
                    _div_pagination.append(pagination);
                }
